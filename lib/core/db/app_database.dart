...
class AppDatabase extends _$AppDatabase {
  ...
  late final PromoDao promoDao = PromoDao(this);
  late final ReportsDao reportsDao = ReportsDao(this);
  late final ScheduleDao scheduleDao = ScheduleDao(this);
  late final SyncDao syncDao = SyncDao(this);
  late final VisitDao visitDao = VisitDao(this);
  ...
  Future<int> rawInsert(String sql, {List<Object?>? whereArgs}) async {
    await customStatement(
      sql,
      whereArgs?.map((v) => v?.toString() ?? null).cast<String>().toList() ??
          [],
    );
    return 1;
  }
  ...
  Future<int> rawDelete(String sql, {List<Object?>? whereArgs}) async {
    await customStatement(
      sql,
      whereArgs?.map((a) => a?.toString() ?? null).cast<String>().toList() ??
          [],
    );
    return 1;
  }
