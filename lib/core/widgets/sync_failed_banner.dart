...
// SyncFailedBanner
//
// Widget untuk menampilkan peringatan ke user ketika ada data yang gagal
// disinkronkan setelah melewati batas retry (5x).
//
// Cara pakai: taruh di bawah AppBar di halaman utama (misal dashboard):
//   const SyncFailedBanner()
// ─────────────────────────────────────────────────────────────────────────────

final _failedItemsProvider = failedSyncItemsProvider;

class SyncFailedBanner extends ConsumerWidget {
  const SyncFailedBanner({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final failedAsync = ref.watch(_failedItemsProvider);
            ...
            child: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      '${failedItems.length} data gagal dikirim. '
                      'Tap untuk detail.',
                  ...
                  GestureDetector(
                    onTap: () =>
                        _showFailedItemsSheet(context, ref, failedItems),
                    child: const Icon(
                      Icons.chevron_right,
                      color: Color(0xFF856404),
                    ),
  ...
  void _showFailedItemsSheet(
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) => _FailedItemsSheet(items: items, ref: ref),
    );
  }
}

class _FailedItemsSheet extends StatelessWidget {
  final List<Map<String, dynamic>> items;
  final WidgetRef ref;

  const _FailedItemsSheet({required this.items, required this.ref});
  ...
  void _editPhoneNumber(
    ...
    // Controller untuk mengisi nilai lama
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              // Copy the map to ensure it's mutable
